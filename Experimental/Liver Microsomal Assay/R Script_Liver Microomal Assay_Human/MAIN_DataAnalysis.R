...
# read file
dataset <- read_excel("Data_Liver Microsomal Assay_Human.xlsx", 
...
dataset <- subset(dataset, dataset$Species == "human")
...
#human
dataset$ModelSD[which(dataset$Species == "human")] <- est_hat["sd"]
dataset$InhibConc[which(dataset$Species == "human")] <- est_hat["EC50"]
dataset$MaxDepletionRate[which(dataset$Species == "human")] <- est_hat["Vmax"]
dataset$MichaelisMentenConstant[which(dataset$Species == "human")]<- est_hat["Km"]
...
plot <-
  ...
  scale_y_log10(
    limits = ylimits)+
  annotate("text", x = Inf-1, y = Inf-1, label = "Human liver microsomes",
           size=10, hjust=1.1, vjust=1.3)+
  annotation_logticks(sides="l",short = unit(2.5,"mm"),mid = unit(5,"mm"))

plot
ggsave("plotlivermicrosomes_human.png", bg="white", width = 9, height = 9)
