...
dataset_rifampicin <- subset(dataset_rifampicin,
                             ID=="Rifampicin_human_gut_600mg_po_MD")


#--- TRANSFORM UNITS 

dataset_rifampicin$Y <- NA
dataset_rifampicin$Y <- dataset_rifampicin$YORIGINAL*1.02 # fraction of gut intracellular space * Vgut [L]
...
for(row in 1:nrow(dataset_rifampicin)){
  ...
  if(dataset_rifampicin$ID[row]=="Rifampicin_human_gut_600mg_po_MD"){
...
SSR_RIF <- sum(dataset_rifampicin_pred$SQUAREDRESIDUALS[which(
  (!is.na(dataset_rifampicin_pred$SQUAREDRESIDUALS)) & 
    ( (dataset_rifampicin_pred$ID=="Rifampicin_human_gut_600mg_po_MD") )
)]
)
...
rifampicin <- 
  ...
  geom_line(aes(x=XORIGINAL/24, y=Y/1.02),size=1.5)+ #divided by liver volume
  geom_line(data=dataset2,aes(x=XORIGINAL/24, y=Y/1.02),size=1.5)+
  ...
  annotate("text", x = Inf, y = Inf, label = "Rifampicin (small intestine intracel.) ",size=11, hjust=1.05, vjust=1.5)
