...
#--- SOLVE ODE SYSTEM FOR RUNTABLE AND PROCESS SOLUTION -----------------------
  ...
  for (row in ids) {
    ...
    if(row=="Rifampicin_human_gut_600mg_po_MD")      {YPRED         <- results_wide[[runid]]$RIFcen[is_timepoints]    }
  ...
  if(add_timepoints==TRUE){
    ...
    for (row in ids) {
      ...
      if(row=="Rifampicin_human_gut_600mg_po_MD")       {new_rows$YPRED         <- results_wide[[runid]]$RIFcen[is_timepoints]      }
